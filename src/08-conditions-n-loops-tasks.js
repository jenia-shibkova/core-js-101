...
function doRectanglesOverlap(rect1, rect2) {
  let result;
  if ((rect1.height < rect2.top || rect2.left > rect1.width)
    || (rect2.height < rect1.top || rect1.left > rect2.width)) {
    result = false;
  } else {
    result = true;
  }
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  let result;
  if (a > b) {
    result = `${startbrackets[isStartIncluded]}${b}, ${a}${endbrackets[isEndIncluded]}`;
  } else {
    result = `${startbrackets[isStartIncluded]}${a}, ${b}${endbrackets[isEndIncluded]}`;
  }
  return result;
}
